<template>
  <div class="app-container">
    ...
    <el-form :inline="true" class="demo-form-inline">
      ...
      <el-form-item>
        <el-select placeholder="发布状态" v-model="searchObj.status">
...
export default {
  ...
  methods: {
    fetchData(page = 1) { // 调用api层获取数据库中的数据
      ...
      course.getCourseListByCondition(this.page, this.limit, this.searchObj).then(res => {
        // debugger 设置断点调试
        if (res.success === true) {
    ...
    subjectLevelOneChanged(value) {
      for (let i = 0; i < this.subjectNestedList.length; i++) {
        if (this.subjectNestedList[i].id === value) {
          this.subSubjectList = this.subjectNestedList[i].subjectsTreeChild
          console.log(this.subjectNestedList[i].subjectsTreeChild)
    ...
    removeCourse(courseId) {
      this.$confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        return course.deleteCourseById(courseId);
      }).then(() => {
        this.fetchData();
