      ...
      <div class="more-sign">
        ...
        <ul>
          <li><a id="weixin" class="weixin" target="_blank"
                 @click="WxLogin"><i class="iconfont icon-weixin"/></a>
          </li>
...
export default {
  ...
  methods: {
    WxLogin() {
      loginApi.getWXLoginURL().then(res => {
    ...
    submitLogin() {
      loginApi.submitLogin(this.user).then(response => {
        if (response.data.success) {
          ...
          cookie.set('lita_token', response.data.data.token, {domain: '8.130.114.187'})
          //登录成功根据token获取用户信息
          loginApi.getLoginInfo().then(response => {

            this.loginInfo = response.data.data.item
            //将用户信息记录cookie
            cookie.set('lita_ucenter', JSON.stringify(this.loginInfo), {domain: '8.130.114.187'})
            //跳转页面
